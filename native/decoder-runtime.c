/*
 * Fixed-memory allocator/runtime derived from engine-web-adapter's
 * decoder/freestanding.c at commit
 * 73e891df62375d1ee45d8228029796ef9597cb43. The package-local copy keeps the
 * decoder module free of WASI, filesystem, and Emscripten JavaScript runtime
 * imports. Changes here add no product or engine behavior.
 */
#include <stddef.h>
#include <stdint.h>
...
#define BLOCK_MAGIC 0x434f4443u

/* libFLAC's decoder finish path compares its FILE handle with stdin. */
...
static void trim_frontier(void) {
  for (;;) {
    ...
    while (block != NULL &&
           (uintptr_t)block + sizeof(Block) + block->size != heap_next) {
      link = &block->next;
      block = block->next;
    }
...
void *malloc(size_t bytes) {
  ...
  if (block != NULL) {
    *link = block->next;
    ...
    block->free = 0;
    block->next = NULL;
    record_allocation(block->size);
    return (void *)(block + 1);
  }
  if (heap_next == 0) heap_next = heap_start();
  start = heap_next;
  if (start > HEAP_LIMIT - sizeof(Block) ||
      wanted > HEAP_LIMIT - start - sizeof(Block)) return NULL;
  block = (Block *)start;
  block->size = wanted;
  block->next = NULL;
...
void free(void *pointer) {
  ...
  *link = block;
  if (block->next != NULL &&
      (uintptr_t)block + sizeof(Block) + block->size ==
          (uintptr_t)block->next) {
    block->size += sizeof(Block) + block->next->size;
    block->next = block->next->next;
  }
  if (previous != NULL &&
      (uintptr_t)previous + sizeof(Block) + previous->size ==
          (uintptr_t)block) {
...
void *realloc(void *pointer, size_t bytes) {
  ...
  for (index = 0; index < copied; index++)
    result[index] = ((unsigned char *)pointer)[index];
  free(pointer);
  return result;
}
...
void *memmove(void *destination, const void *source, size_t bytes) {
  ...
  if (out < in) {
    ...
    for (index = bytes; index != 0; index--) out[index - 1] = in[index - 1];
  }
  return destination;
}
...
uint32_t codec_decoder_allocator_live_bytes(void) {
  return (uint32_t)live_bytes;
}

uint32_t codec_decoder_allocator_peak_live_bytes(void) {
  return (uint32_t)peak_live_bytes;
}

uint32_t codec_decoder_allocator_peak_heap_bytes(void) {
  return peak_heap_end == 0
      ? 0u
      : (uint32_t)(peak_heap_end - heap_start());
}

uint32_t codec_decoder_allocator_free_calls(void) {
  return free_calls;
}

uint32_t codec_decoder_allocator_realloc_calls(void) {
